...
def main(plot = False):
    ...
    csv_path = os.path.join(abs_path, '..', 'Node_Info.csv')
    swc_path = os.path.join(abs_path, '..', 'After_stretch.swc')
    
    forest = SWC2Forest(testfile_path)
    print_forest_to_CSVFile(forest, csv_path)
    print_forest_to_SWCFile(forest, swc_path)
    if plot == True:
        PlotForest(forest)
